...
def time1(update:Update, context: CallbackContext) -> None:
    ...
    if (sh.days_int[f'{update.message.text}'] < current_day_of_week+1):
        update.message.reply_text('You cannot add reservations to the previous days of the week', reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END
    ...
    for i in time:
        if i[-1] == update.message.text:
            if i[-1][0]*10+i[-1][1]<time_string[0]*10+time_string[1] and (current_day_of_week==sh.days_int[f"{new_slot['day']}"]-1):
                update.message.reply_text('You cannot add outdated slots', reply_markup=ReplyKeyboardRemove())
                return ConversationHandler.END
            update.message.reply_text(sh.add(new_slot['username'], new_slot['day'], time.index(i) + 9), reply_markup=ReplyKeyboardRemove())
    new_slot.clear()
    return ConversationHandler.END
...
def delete_time(update:Update, context: CallbackContext) -> None:
    ...
    current_day_of_week = current_datetime.weekday()
    if (sh.days_int[f'{update.message.text}'] < current_day_of_week+1):
        update.message.reply_text('You cannot cancel outdated reservations', reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END
    update.message.reply_text(sh.get_list_day(update.message.text))
    ...
    for i in time:
        if i[-1] == update.message.text:
            ...
            if i[-1][0]*10+i[-1][1]<=time_string[0]*10+time_string[1] and (current_day_of_week==sh.days_int[f"{del_slot['day']}"]-1):
                update.message.reply_text('You cannot cancel outdated reservations', reply_markup=ReplyKeyboardRemove())
                return ConversationHandler.END
            update.message.reply_text(sh.delete(update, del_slot['username'], del_slot['day'], time.index(i) + 9), reply_markup=ReplyKeyboardRemove())
    del_slot.clear()
    return ConversationHandler.END
